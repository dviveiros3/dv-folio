# Growth Audit: Launch Week Operating Runbook

## Mission
Operationalize the Growth Audit paid beta launch. This runbook ensures Daniel focuses on generating and capturing market signal through a disciplined operating cadence, avoiding scattered activity and preserving brand quality.

## Core Operational Links
- **Launch Strategy:** [Growth Distribution Sprint](../sprints/growth-distribution-sprint.md)
- **Content:** [Launch Post Pack](./growth-audit-launch-post-pack.md)
- **Triage:** [Post-Launch Triage Guide](./growth-audit-post-launch-triage-guide.md)
- **Evidence:** [Launch Evidence Capture Checklist](./growth-audit-launch-evidence-capture-checklist.md)
- **Decision:** [Beta Decision Dashboard](./growth-audit-beta-decision-dashboard.md)
- **Capture:** [Beta Conversation Tracker](./growth-audit-beta-conversation-tracker.md)

## Daily Operating Cadence (Time Blocks)
- **08:30 - 09:00: Publish & Seed.** Execute the day's post. Engage with initial comments.
- **12:00 - 12:30: Noon Triage.** Reply to comments. Move high-signal engagers to DMs.
- **16:30 - 17:15: Deep Triage & Evidence Capture.** Process all DMs. Update the [Conversation Tracker](./growth-audit-beta-conversation-tracker.md). Log exact buyer language and objections.
- **17:15 - 17:30: Daily Pulse Check.** Review signal level. Adjust tomorrow's derivative post if needed.

---

## The Weekly Schedule

### Monday: The Flagship Launch
- **Action:** Publish Post 1 (The Flagship Announcement).
- **Focus:** Broad POV on "System over Slop" and the "Engineering Diagnostic."
- **Proof-Safety:** Ensure no revenue guarantees. Use "designed to" framing.
- **Signal Goal:** Identify whether any "Efficiency-Seeker" or "Technical Founder" profiles appear in the engagement list.

### Tuesday: The Signal Response
- **Action:** Publish Post 2 (Judgment vs. Automation).
- **Focus:** Address the "AI Slop" pain point. Pivot to the Distribution OS framework.
- **Proof-Safety:** Frame AI as leverage for founder judgment, not a replacement.
- **Signal Goal:** Capture any qualified DM language that confirms or challenges the "AI as leverage for judgment" thesis.

### Wednesday: The Engine Room
- **Action:** Publish Post 3 (Artifact-Led / Evidence).
- **Focus:** Show the internal rigor (e.g., Outreach Worksheet or Deliverable Walkthrough).
- **Proof-Safety:** Label all artifacts as "Simulated" or "Sample." Redact real names/companies.
- **Signal Goal:** Watch for requests for the "Audit Specs" or "Sample Walkthrough."

### Thursday: Warm Outbound & Follow-ups
- **Action:** Direct Outreach / Manual Follow-ups.
- **Focus:** Reach out to high-signal engagers from Mon-Wed who haven't moved to DM yet.
- **Proof-Safety:** Personalized, low-pressure, operator-to-operator tone.
- **Signal Goal:** Determine whether high-signal engagers are ready for a discovery call or need the sample deliverable first.

### Friday: Offer Intelligence & Decision
- **Action:** Weekly Offer Review & Dashboard Update.
- **Focus:** Run the [20-minute review sequence](./growth-audit-beta-decision-dashboard.md#4-friday-20-minute-review-sequence). Make the "Matrix Action" decision.
- **Time Block:** 09:00 - 10:00 (Extended review).
- **Signal Goal:** Decide whether exact buyer language from the week is strong enough to update the Growth Audit offer.

---

## Signal Contingency Logic

### If No Signal (0-24 Hours)
- **Diagnosis:** Hook or timing failure.
- **Action:** Do not panic. Engage with peers/validators to boost reach. Check if the "Flagship" CTA was too heavy (e.g., asking for a call vs. a DM).
- **Adjustment:** For Tuesday, use a shorter, more provocative hook. Use the "Medium Signal" triage path.

### If No Signal (48-72 Hours)
- **Diagnosis:** Thesis-Market mismatch or ICP misalignment.
- **Action:** Pause standard posting. Run 5-10 manual outbound messages to friendly ICPs asking for "Feedback on the diagnostic specs" rather than a sale.
- **Dashboard Trigger:** Prepare to "Change CTA" or "Change ICP" in the Friday review.

### If Strong Signal
- **Diagnosis:** The launch is producing qualified buyer language worth prioritizing.
- **Action:** Prioritize DMs over public replies. Stick to the [Discovery Call Script](./growth-audit-discovery-call-script.md).
- **Caution:** Do not over-book. Maintain the 1-week diagnostic timeline promise.
- **Dashboard Trigger:** "Continue" or "Raise Price" if friction is zero.

---

## Proof-Safety Guardrails (Mandatory Check)
*Before every post or reply, ask:*
1. Am I promising a specific result (leads/revenue)? **(Stop if Yes)**
2. Is this artifact labeled "Sample" or "Simulated"? **(Must be Yes if not a real client)**
3. Am I faking scarcity or urgency? **(Stop if Yes)**
4. Does this sound like a "Magic Button" or an "Engineering Diagnostic"? **(Must be Diagnostic)**

---

## Launch Week Scorecard
*Update daily. No fake metrics.*

| Metric | Mon | Tue | Wed | Thu | Fri | Total |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| Posts Shipped | | | | | | 0 |
| Total Comments | | | | | | 0 |
| High-Signal Replies | | | | | | 0 |
| DMs Initiated | | | | | | 0 |
| Discovery Calls Booked| | | | | | 0 |
| "Yes" Decisions | | | | | | 0 |
| Objections Logged | | | | | | 0 |

---
**Next Step:** Follow the [Friday Review Sequence](./growth-audit-beta-decision-dashboard.md#4-friday-20-minute-review-sequence).
